...
const createTrapping = (request, response) => {
    const {title,remarks,zonaid,authorityid,is_active,is_visible,type_of_shape} = request.body
    pool.query('INSERT INTO tbl_marlens_trapping (title,remarks,zonaid,authorityid,is_active,is_visible,type_of_shape) VALUES ($1,$2,$3,$4,$5,$6,$7)', [title,remarks,zonaid,authorityid,is_active,is_visible,type_of_shape], (error, results) => {
      if (error) {
        if (error.code == '23505')
        {
...
const readTrapping = (request, response) => {
    ...
    pool.query('SELECT count(*) as total FROM tbl_marlens_trapping WHERE is_delete=false and is_active=true or is_visible=true', (error, results) => {
      ...
      var sql=  'SELECT *,row_number() OVER (ORDER BY tbl_marlens_trapping.id) AS row_number FROM tbl_marlens_trapping WHERE is_delete=false and is_active=true or is_visible=true ORDER BY id ASC'
      pool.query(
       sql,
        (error, results) => {
          if (error) {
...
const updateTrapping = (request, response) => {
    ...
    pool.query('SELECT * FROM tbl_marlens_trapping WHERE id=$1', [id],(error, results) => {
      ...
      if (results.rowCount >0){
        ...
        pool.query('UPDATE tbl_marlens_trapping set title=$1,remarks=$2,zonaid=$3,authorityid=$4,is_active=$5,is_visible=$6,type_of_shape=$7,updated_at=$8 WHERE id=$9', [title,remarks,zonaid,authorityid,is_active,is_visible,type_of_shape,update_time,id], (error, results) => {
          if (error) {
            response.status(400).send({success:false,data: error})
            return;
          }
