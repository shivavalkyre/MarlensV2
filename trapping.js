...
const readTrapping = (request, response) => {
    ...
    pool.query('SELECT count(*) as total FROM tbl_marlens_trapping WHERE is_delete=false and is_active=true or is_visible=true', (error, results) => {
      ...
      var sql=  'SELECT m.id,m.created_at,m.is_active,m.is_visible, k.ship_name, k.call_sign, k.mmsi, k.gt, j.ship_type FROM tbl_marlens_trapping m join tbl_masdex_kapal k on m.select_ship = k.id join tbl_masdex_jenis_kapal j on k.ship_type = j.id WHERE m.is_delete=false and (m.is_active=true or m.is_visible=true) ORDER BY m.id ASC'
      pool.query(
       sql,
        (error, results) => {
          if (error) {
